...
const AddStore = () => {
  ...
  const inventoriesStatus = useSelector((state) => state.admin.inventoryStatus);
  ...
  const handleBranchChange = (e) => {
    setFormData({ ...formData, branches: e.target.value.split(',') });
  };
  
  const handleInventoryChange = (selectedList) => {
    setFormData({
      ...formData,
      inventories: selectedList.map((inventory) => inventory._id),
    });
  };
      ...
      <div className="w-full max-w-md p-8 bg-primary rounded-lg shadow-lg">
        ...
        <form className="w-full" onSubmit={handleSubmit}>
         ...
         <InputField
          ...
          type="text"
          id="branches"
          placeholder="Enter branches separated by comma"
...
<div className="mt-4">
          <label htmlFor="inventories" className="block text-sm font-medium text-highlight">
            Inventories
          </label>
          <Multiselect
            displayValue="name"
