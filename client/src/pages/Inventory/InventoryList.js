...
import { fetchInventories, deleteInventory } from '../../redux/slices/inventorySlice/inventorySlice';
...
const InventoryList = () => {
  ...
  const handleEdit = (inventory) => {
    navigate('/dashboard/manage-inventory', { state: { inventory } });
  };

  const handleDelete = (id) => {
    setInventoryToDelete(id);
    setPopupMessage('Are you sure you want to delete this inventory?');
    setConfirmationPopup(true);
  };

  const confirmDelete = async () => {
    try {
      await dispatch(deleteInventory(inventoryToDelete)).unwrap();
      setPopupMessage('Inventory deleted successfully');
    } catch (error) {
      setPopupMessage('Failed to delete inventory. Please try again.');
    }
  ...
  const closePopup = () => {
    ...
    if (!confirmationPopup && inventoryToDelete) {
      dispatch(fetchInventories());
    }
  };
      ...
      <div className="flex mb-6">
        <div className="flex bg-slate-300 items-center rounded-lg p-3 w-4/5">
          ...
          <input
            ...
            className="ml-3 outline-none w-full bg-slate-300"
          />
        </div>
        <button
      ...
      <div className="overflow-x-auto bg-gray-50 rounded-lg shadow-lg">
        <table className="w-full min-w-max bg-white text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="4" className="p-4 text-center"><RingLoader /></td>
              </tr>
            ) : error ? (
              <tr>
                <td colSpan="4" className="p-4 text-center text-red-500">{error}</td>
              </tr>
            ) : filteredInventories.length === 0 ? (
              <tr>
                <td colSpan="4" className="p-4 text-center">No inventories found</td>
              </tr>
            ) : (
              filteredInventories.map((inventory) => (
                ...
                <button onClick={() => handleEdit(inventory)} aria-label="Edit">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                ...
                <button onClick={() => handleDelete(inventory._id)} aria-label="Delete">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
